...
def Extraction_Per(doc, listePersonnages, listeFiltre, listeRetirer):
    for ent in doc.ents:
        if ent.type == "PER" and len(ent.text) > 2:
            ent_cleaned = ent.text.replace("\n", " ").strip()
            if ent_cleaned not in listeFiltre:
                listePersonnages.append(ent_cleaned)
            else:
                listeRetirer.append(ent_cleaned)
...
def Ranger_Par_Noms(listePersonnagesTrier,listeNomsPersonnages):                                    
    for nom in listePersonnagesTrier:
        variantesNoms = {nom}  # Utiliser un set pour stocker les variantes
        for autreNom in listePersonnagesTrier:
            if len(nom.split()) >= 2 and autreNom != nom and (autreNom.split()[-1] in nom.split() or nom.split()[-1] in autreNom.split()):
                variantesNoms.add(autreNom)
        ...
        if not any(sorted(variantesNoms) == sorted(existing) for existing in listeNomsPersonnages):
            listeNomsPersonnages.append(list(variantesNoms))
...
for chapters, book_code in books:
    ...
    for chapter in chapters:
        ...
        Extraction_Per(doc,listePersonnages,listeFiltre,listeRetirer)
